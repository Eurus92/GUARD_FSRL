...
from fsrl.utils.wrapper.subproc import (
    # DummyEnvWorker,
    EnvWorker,
    # RayEnvWorker,
    SafeSubprocEnvWorker,
)
...
def _patch_env_generator(fn: Callable[[], ENV_TYPE]) -> Callable[[], gym.Env]:
    ...
    def patched() -> gym.Env:
        ...
        env = fn()
        if isinstance(env, (gym.Env, Engine)):  # modif
            return env
...
class BaseVectorEnv(object):
    ...
    def __init__(
        ...
        worker_fn: Callable[[Callable[[], Engine]], EnvWorker],  # modif
    ...
    def step(
        ...
        obs_list, rew_list, term_list, trunc_list = tuple(zip(*result))
        try:
...
class SafeShmemVectorEnv(BaseVectorEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], ENV_TYPE]], **kwargs: Any) -> None:
        def worker_fn(fn: Callable[[], Engine]) -> SafeSubprocEnvWorker:
            return SafeSubprocEnvWorker(fn, share_memory=True)
